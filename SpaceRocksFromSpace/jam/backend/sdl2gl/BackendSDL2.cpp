...
namespace jam
{
  ...
  bool BackendSDL2::Construct(std::string title, int screenWidth, int screenHeight)
  {
	...
	success = false;
      ...
      {
	//Create window
	this->window = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screenWidth, screenHeight, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
	if (this->window == NULL)
	  {
  ...
  {       
    ...
    render->Init(this->window, &this->context);
